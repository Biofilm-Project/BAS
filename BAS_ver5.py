...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def set_line(self):
        pass
        #w = SliderWindow(self.conv_img)
        #w.show()

        #self.threshold = int(SliderWindow.slider.value())
        ...
        #self.ax.hist(np.array(self.image_color).ravel(), 256, [1,256])
        #self.ax.axvline(int(self.slider.value()), color = 'k', linestyle= 'dashed', linewidth=1)
        #plt.show()
    ...
    def thresholding(self):
        ...
        #denoised_img = cv2.GaussianBlur(self.image_color, (9,9),0)
        #denoised_img2 = cv2.GassianBlur(self.image_color2, (9,9),0)
        
        #Contrast stretching --> Percentile Stretching np.percentile(a, 50)
        #Creating zero array
        #empty = np.zeros((denoised_img.shape[0], denoised_img.shape[1]), dtype = 'uint8')
        #empty2 = np.zeros((denoised_img2.shape[0], denoised_img2.shape[1]), dtype = 'uint8')
        #Looping over
        #for i in range(denoised_img.shape[0]):
         #   for j in range(denoised_img.shape[1]):
          #      QtWidgets.QApplication.processEvents()
           #     empty[i,j] = 255*(denoised_img[i,j]-np.percentile(denoised_img,5))/(np.percentile(denoised_img,95)-np.percentile(denoised_img,5))

        #for i in range(denoised_img2.shape[0]):
         #   for j in range(denoised_img2.shape[1]):
          #      QtWidgets.QApplication.processEvents()
           #     empty2[i,j] = 255*(denoised_img2[i,j]-np.percentile(denoised_img2,5))/(np.percentile(denoised_img2,95)-np.percentile(denoised_img2,5))

        #self.image_color = denoised_img
        #self.image_color2 = denoised_img2

        #print("B")
        self.slider_window = SliderWindow(self.image_color)
        self.slider_window.show()
        
        #print (self.slider_window.slider_value)
        #if self.slider_window.slider_value != None:
        self.fourth_step.setStyleSheet("border: 1px solid black")
    ...
    def show_results(self):
        ...
        self.temp.append(wh_mean)
        self.temp.append(res_mean)
        self.temp.append(percentage)
        self.temp.append(0)
    ...
    def add_result(self):
        self.ref_mean.append(self.temp[0])
        self.bio_mean.append(self.temp[1])
        self.proportion.append(self.temp[2])
        self.ref_names.append(self.just_b_filename[self.ref_index].split('.')[0])
        self.bio_names.append(self.just_c_filename[self.bio_index].split('.')[0])
        self.just_zero.append(0)
    ...
    def export_results(self):
        order_names = self.ref_names+self.bio_names
        proportions = self.just_zero+self.proportion
        means = self.ref_mean+self.bio_mean
        tipo = self.just_zero + [1 for x in range(len(self.just_zero))]
        ...
        print(self.directory)
        print(experimento)
        filename = r''+self.location+experimento+'_results.xlsx'
        filename = filename.replace("//","\\")
        print(filename)
        df = pd.DataFrame({'Well':order_names, 'M.C':means, 'Proportion':proportions, 'Color Scale':tipo})
